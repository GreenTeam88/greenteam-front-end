...
interface StepProps {
  ...
  formData: {
    ...
    stepFivePriceSchilderen?: number;

    totalCost?: number;

    // Remember input data by the user
    zijkantenSchilderenService?: string;
    DemontageSchilderenService?: string;
  };
...
const schema = z
  .object({
    ...
    zijkantenSchilderenService: z.string().optional(),
    DemontageSchilderenService: z.string().optional(),
  })
  .superRefine((data, ctx) => {
    const { closeStairsCount } = data;

    // If user has closeStairs > 0 => must choose "Ja" or "Nee" for zijkantenService
    if (closeStairsCount > 0 && !data.zijkantenSchilderenService) {
      ctx.addIssue({
        code: 'custom',
        path: ['zijkantenSchilderenService'],
    ...
    if (closeStairsCount > 0 && !data.DemontageSchilderenService) {
      ctx.addIssue({
        code: 'custom',
        path: ['DemontageSchilderenService'],
...
const StepFive: React.FC<StepProps> = ({ onPrevious, onNext, formData, updateFormData }) => {
  ...
  // Watch fields
  const watchZijkantenService = useWatch({ control, name: 'zijkantenSchilderenService' }) || '';
  const watchDemontageService = useWatch({ control, name: 'DemontageSchilderenService' }) || '';
  ...
  useEffect(() => {
    ...
    let newstepFivePriceSchilderen = 0;
    ...
    if ((openStairsCount > 0 || closeStairsCount > 0) && watchZijkantenService === 'Ja') {
      ...
      newstepFivePriceSchilderen += zijkantenPrice * (closeStairsCount + openStairsCount);
    }
    ...
    if ((openStairsCount > 0 || closeStairsCount > 0) && watchDemontageService === 'Ja') {
      ...
      newstepFivePriceSchilderen += demontagePrice * (closeStairsCount + openStairsCount);
    }

    // Calculate new totalCost by subtracting old stepFivePriceSchurenBetonCire and adding newStepFivePrice
    const oldStepFivePrice = formData.stepFivePriceSchilderen || 0;
    const oldTotal = formData.totalCost || 0;
    const newTotal = oldTotal - oldStepFivePrice + newstepFivePriceSchilderen;
    setStepFivePrice(newTotal);

    // Only update if there is a change to prevent infinite loop
    if (newstepFivePriceSchilderen !== oldStepFivePrice) {
      updateFormData({
        ...formData,
        stepFivePriceSchilderen: newstepFivePriceSchilderen,
        zijkantenSchilderenService: watchZijkantenService,
        DemontageSchilderenService: watchDemontageService,
        totalCost: newTotal,
      });
    }
  }, [
    ...
    formData.stepFivePriceSchilderen,
    formData.totalCost,
    updateFormData,
  ]);
        ...
        <div className="bg-white w-full rounded-b-[8px] flex flex-col px-[22px] py-[22px] gap-y-3 shadow-md">
          ...
          <div className="flex flex-col gap-[11px]">
            <SingleSelectDropdown
              ...
              name="zijkantenSchilderenService"
              label="Wilt u ook dat wij de zijkanten bekleden?"
              placeholder="Maak een keuze"
            />
            {errors.zijkantenSchilderenService && (
              <p className="text-red-600 text-sm">{errors.zijkantenSchilderenService.message}</p>
            )}
          </div>
          ...
          <div className="flex flex-col gap-[11px]">
            <SingleSelectDropdown
              ...
              name="DemontageSchilderenService"
              label="Wilt u dat wij ook demontage uitvoeren?"
              placeholder="Maak een keuze"
            />
            {errors.DemontageSchilderenService && (
              <p className="text-red-600 text-sm">{errors.DemontageSchilderenService.message}</p>
            )}
          </div>
