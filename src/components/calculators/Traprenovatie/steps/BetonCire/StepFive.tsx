...
interface StepProps {
  ...
  formData: {
    ...
    StepFivePriceBetonCire?: number;
    totalCost?: number;

    // Remember input data by the user
    zijkantenBetonCireService?: string;
    DemontageBetonCireService?: string;
  };
...
const schema = z
  .object({
    ...
    zijkantenBetonCireService: z.string().optional(),
    DemontageBetonCireService: z.string().optional(),
  })
  .superRefine((data, ctx) => {
    const { closeStairsCount } = data;

    // If user has closeStairs > 0 => must choose "Ja"/"Nee" for zijkantenBetonCireService
    if (closeStairsCount > 0 && !data.zijkantenBetonCireService) {
      ctx.addIssue({
        code: 'custom',
        path: ['zijkantenBetonCireService'],
        message: 'Geef aan of de zijkanten ook bekleed moeten worden.',
      });
    }

    // If user has closeStairs > 0 => must choose "Ja"/"Nee" for DemontageBetonCireService
    if (closeStairsCount > 0 && !data.DemontageBetonCireService) {
      ctx.addIssue({
        code: 'custom',
        path: ['DemontageBetonCireService'],
...
const StepFive: React.FC<StepProps> = ({ onPrevious, onNext, formData, updateFormData }) => {
  ...
  // Watch fields
  const watchZijkantenService = useWatch({ control, name: 'zijkantenBetonCireService' }) || '';
  const watchDemontageService = useWatch({ control, name: 'DemontageBetonCireService' }) || '';

  // We'll track the new total for display
  ...
  useEffect(() => {
    ...
    let newStepFivePriceBetonCire = 0;
    ...
    if ((openStairsCount > 0 || closeStairsCount > 0) && watchZijkantenService === 'Ja') {
      ...
      newStepFivePriceBetonCire += zijkantenPrice * (closeStairsCount + openStairsCount);
    }
    ...
    if ((openStairsCount > 0 || closeStairsCount > 0) && watchDemontageService === 'Ja') {
      ...
      newStepFivePriceBetonCire += demontagePrice * (closeStairsCount + openStairsCount);
    }

    // old partial cost & old total
    const oldStepFivePrice = formData.StepFivePriceBetonCire || 0;
    const oldTotal = formData.totalCost || 0;

    // new total = oldTotal - oldPartial + new partial
    const newTotal = oldTotal - oldStepFivePrice + newStepFivePriceBetonCire;
    setStepFivePrice(newTotal);

    // Only update formData if there's a real change
    if (newStepFivePriceBetonCire !== oldStepFivePrice) {
      updateFormData({
        ...formData,
        StepFivePriceBetonCire: newStepFivePriceBetonCire,
        zijkantenBetonCireService: watchZijkantenService,
        DemontageBetonCireService: watchDemontageService,
        totalCost: newTotal,
      });
    }
  }, [
    ...
    formData.StepFivePriceBetonCire,
    formData.totalCost,
    updateFormData,
  ]);
        ...
        <div className="bg-white w-full rounded-b-[8px] flex flex-col px-[22px] py-[22px] gap-y-3 shadow-md">
          ...
          <div className="flex flex-col gap-[11px]">
            <SingleSelectDropdown
              ...
              name="zijkantenBetonCireService"
              label="Wilt u ook dat wij de zijkanten bekleden?"
              placeholder="Maak een keuze"
            />
            {errors.zijkantenBetonCireService && (
              <p className="text-red-600 text-sm">{errors.zijkantenBetonCireService.message}</p>
            )}
          </div>
          ...
          <div className="flex flex-col gap-[11px]">
            <SingleSelectDropdown
              ...
              name="DemontageBetonCireService"
              label="Wilt u dat wij ook demontage uitvoeren?"
              placeholder="Maak een keuze"
            />
            {errors.DemontageBetonCireService && (
              <p className="text-red-600 text-sm">{errors.DemontageBetonCireService.message}</p>
            )}
          </div>
